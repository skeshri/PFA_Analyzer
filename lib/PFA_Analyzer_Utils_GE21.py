...
iPhi_boundary1 = -0.05843272 
iPhi_boundary2 = -0.02913321
iPhi_boundary3 = 0.00068867
iPhi_boundary4 = 0.03010412
iPhi_boundary5 = 0.05975302
mapping_GE21 = {(6,1,2):0, (6,3,4):1, (5,1,2):2, (5,3,4):3, (4,1,2):4, (4,3,4):5, (3,1,2):6, (3,3,4):7, (2,1,2):8, (2,3,4):9, (1,1,2):10, (1,3,4):11,       (6,5,6):0, (6,7,8):1, (5,5,6):2, (5,7,8):3, (4,5,6):4, (4,7,8):5, (3,5,6):6, (3,7,8):7, (2,5,6):8, (2,7,8):9, (1,5,6):10, (1,7,8):11, (6,9,10):0, (6,11,12):1, (5,9,10):2, (5,11,12):3, (4,9,10):4, (4,11,12):5, (3,9,10):6, (3,11,12):7, (2,9,10):8, (2,11,12):9, (1,9,10):10, (1,11,12):11, (6,13,14):0, (6,15,16):1, (5,13,14):2, (5,15,16):3, (4,13,14):4, (4,15,16):5, (3,13,14):6, (3,15,16):7, (2,13,14):8, (2,15,16):9, (1,13,14):10, (1,15,16):11 }
...
def iEta_iPhi2VFAT(iEta,iPhi):
    ...
    VFAT = -1
    #print(f'iEta : {iEta}, iPhi : {iPhi}')
    for k,v in mapping_GE21.items():
        if (iPhi == k[0] and (iEta == k[1] or iEta == k[2])):
            VFAT = v
            break
...
def get_iPhi(cos_phi):
    iphi=-1
    if cos_phi < iPhi_boundary1:
        return 1
    elif cos_phi > iPhi_boundary1 and cos_phi < iPhi_boundary2:
        return 2
    elif cos_phi > iPhi_boundary2 and cos_phi < iPhi_boundary3:
        return 3
    elif cos_phi > iPhi_boundary3 and cos_phi < iPhi_boundary4:
        return 4
    elif cos_phi > iPhi_boundary4 and cos_phi < iPhi_boundary5:
        return 5
    elif cos_phi > iPhi_boundary5:
        return 6
    return -1

# {(iphi,ieta1,ieta2):vfat }
...
def propHit2VFAT(glb_r,loc_x,etaP):
    ...
    iPhi = get_iPhi(prophit_cosine)
    #print("%%%%% iPhi: ",iPhi)
    if (iPhi != -1):    
        VFAT = iEta_iPhi2VFAT(etaP,iPhi)
    #print(iPhi, etaP, VFAT)
    return VFAT
...
def VFATcompatibleHit(VFATs,glb_r,loc_x,etaP):
    for VFAT in VFATs :
        ...
        phieta = VFAT2iEta_iPhi(VFAT)
        iEta = phieta[1:]
        iPhi = phieta[0]
        if etaP in iEta[1:]:
            if iPhi == 1 and abs(abs(hit_cosine)-abs(iPhi_boundary1)) <= abs(iPhi_boundary1):
                print(iPhi_boundary1,iPhi.etaP)
                return True
            elif iPhi == 2 and ( abs(abs(hit_cosine)-abs(iPhi_boundary1)) > abs(iPhi_boundary1) and abs(abs(hit_cosine)-abs(iPhi_boundary2)) <= abs(iPhi_boundary2) ):
                print(iPhi_boundary2,iPhi,etaP)
                return True
            elif iPhi == 3 and ( abs(abs(hit_cosine)-abs(iPhi_boundary2)) > abs(iPhi_boundary2) and abs(abs(hit_cosine)-abs(iPhi_boundary3)) <= abs(iPhi_boundary3) ):
                print(iPhi_boundary3,iPhi,etaP)
                return True
            elif iPhi == 4 and ( abs(abs(hit_cosine)-abs(iPhi_boundary3)) > abs(iPhi_boundary3) and abs(abs(hit_cosine)-abs(iPhi_boundary4)) <= abs(iPhi_boundary4) ):
                print(iPhi_boundary4,iPhi,etaP)
                return True
            elif iPhi == 5 and ( abs(abs(hit_cosine)-abs(iPhi_boundary4)) > abs(iPhi_boundary4) and abs(abs(hit_cosine)-abs(iPhi_boundary5)) <= abs(iPhi_boundary5) ):
                print(iPhi_boundary5,iPhi,etaP)
                return True
            elif iPhi == 6 and ( abs(abs(hit_cosine)-abs(iPhi_boundary5)) > abs(iPhi_boundary5)) :
                print(iPhi,etaP)
                return True
...
def generateVFATMaskTH2(station=1):
    ...
    for re,la in [(1,1),(1,2),(-1,1),(-1,2)]:
        ...
        output_dict_GE21[endcapTag] = {
                                    1:ROOT.TH2F("GE21-"+endcapTag+" M1 Masked VFAT","GE21-"+endcapTag+" M1 Masked VFAT",18,0.5,18.5,12,-0.5,11.5),
                                    2:ROOT.TH2F("GE21-"+endcapTag+" M2 Masked VFAT","GE21-"+endcapTag+" M2 Masked VFAT",18,0.5,18.5,12,-0.5,11.5),
                                    3:ROOT.TH2F("GE21-"+endcapTag+" M3 Masked VFAT","GE21-"+endcapTag+" M3 Masked VFAT",18,0.5,18.5,12,-0.5,11.5),
                                    4:ROOT.TH2F("GE21-"+endcapTag+" M4 Masked VFAT","GE21-"+endcapTag+" M4 Masked VFAT",18,0.5,18.5,12,-0.5,11.5)
                                    }
    for key_1 in output_dict_GE21:
        for key_2 in output_dict_GE21[key_1]:
            output_dict_GE21[key_1][key_2].GetYaxis().SetTitle("VFAT Number")
            output_dict_GE21[key_1][key_2].GetXaxis().SetTitle("Chamber")
            output_dict_GE21[key_1][key_2].GetXaxis().SetNdivisions(20,0,0,True)
            output_dict_GE21[key_1][key_2].GetYaxis().SetNdivisions(20,0,0,True)
            output_dict_GE21[key_1][key_2].GetYaxis().SetLabelSize(0.02)
            output_dict_GE21[key_1][key_2].GetXaxis().SetLabelSize(0.02)
            output_dict_GE21[key_1][key_2].GetXaxis().SetTickLength(0.005)
            output_dict_GE21[key_1][key_2].GetYaxis().SetTickLength(0.005)
            output_dict_GE21[key_1][key_2].SetStats(False)
            output_dict_GE21[key_1][key_2].SetMinimum(0)
    
    for re,la in [(1,1),(1,2),(-1,1),(-1,2)]:
        endcapTag = EndcapLayer2label(re,la)
        output_dict_GE11[endcapTag] = ROOT.TH2F("GE11-"+endcapTag+"  Masked VFAT","GE11-"+endcapTag+"  Masked VFAT",36,0.5,36.5,24,-0.5,23.5)
    for key_1 in output_dict_GE11.keys():
        output_dict_GE11[key_1].GetYaxis().SetTitle("VFAT Number")
        output_dict_GE11[key_1].GetXaxis().SetTitle("Chamber")
        output_dict_GE11[key_1].GetXaxis().SetNdivisions(40,0,0,True)
        output_dict_GE11[key_1].GetYaxis().SetNdivisions(40,0,0,True)
        output_dict_GE11[key_1].GetYaxis().SetLabelSize(0.02)
        output_dict_GE11[key_1].GetXaxis().SetLabelSize(0.02)
        output_dict_GE11[key_1].GetXaxis().SetTickLength(0.005)
        output_dict_GE11[key_1].GetYaxis().SetTickLength(0.005)
        output_dict_GE11[key_1].SetStats(False)
        output_dict_GE11[key_1].SetMinimum(0)
    
    return output_dict_GE11,output_dict_GE21
...
def generate1DxResidualContainer(matching_variables,nbins,ResidualCutOff):
    ...
    for mv in matching_variables:
        ...
        for (re,la) in [(1,1),(1,2),(-1,1),(-1,2)]:
            ...
            for chamber in range(1,37):
                chID = getChamberName(re,chamber,la,station=2)
...
def whichBitsAreTrue(value,vector_length):
    return np.flatnonzero(np.flip(np.where( np.array(list(np.binary_repr(value,width=vector_length))) == '1', 1,0)))
def whichBitsAreFalse(value,vector_length):
    return np.flatnonzero(np.flip(np.where( np.array(list(np.binary_repr(value,width=vector_length))) == '1', 0,1)))
...
def unpackVFATStatus(evt,VFATMaskBook):
    output_dict_GE11,output_dict_GE21 = VFATMaskBook
    VFATMasked_dict = {}
    for k,layer in enumerate(evt.gemOHStatus_layer):
        station = evt.gemOHStatus_station[k]
        #vector_length = 24 if station == 1 else 12 # Sumit
        vector_length = 12
        region = evt.gemOHStatus_region[k]
        chamberType = evt.gemOHStatus_chamberType[k]
        #print(chamberType)
        chamber = evt.gemOHStatus_chamber[k]
        endcapTag = EndcapLayer2label(region,layer)
        VFATsMasked = whichBitsAreFalse(evt.gemOHStatus_VFATMasked[k],vector_length)
        ...
        chamberType = chamberType % 20

        #if (chamberType in [1,2,3,4,5]):print("chamberType==>>>>>",chamberType)
        # if error or warning, mask the entire chamber
        if (error != 0 or warning != 0) : 
            #VFATsMasked = list(range(24)) if station == 1 else list(range(12)) #Sumit
            VFATsMasked = list(range(12))
            
        ## Not ready for GE21, VFAT range to be checked
        #if station == 1:
        #    for v in VFATsMasked: output_dict_GE11[endcapTag].Fill(chamber,v)
        #    if VFATsMasked != []:
        ...
        if station == 2:
            for v in VFATsMasked: output_dict_GE21[endcapTag][chamberType].Fill(chamber,v)
            #print("=========== VFATMasked =======: ",VFATsMasked)
            if type(VFATsMasked) != list:
                VFATsMasked = list(VFATsMasked)
            if VFATsMasked != []:
                if chamberType == 2: print(chamberType)
                if VFATMasked_dict.get(chamberID) is None:
                    VFATMasked_dict[chamberID] = {chamberType : VFATsMasked}
                elif VFATMasked_dict[chamberID].get(chamberType) is None:
                    VFATMasked_dict[chamberID] = {chamberType : VFATsMasked} 
                else:
                    VFATMasked_dict[chamberID][chamberType] =  np.append(VFATMasked_dict[chamberID][chamberType],VFATsMasked)
    #print(VFATMasked_dict) 
    return VFATMasked_dict,(output_dict_GE11,output_dict_GE21)
...
def generateVFATDict(matching_variables):
    ...
    for mv in matching_variables:
        ...
        for (re,la) in [(1,1),(1,2),(-1,1),(-1,2)]:
            endcap_key = EndcapLayer2label(re,la)
            output_dict[mv][endcap_key] = {}
            #for chamber in range(16,17):
            for chamber in [16, 18]:
                chID = getChamberName(re,chamber,la,station=2)
                output_dict[mv][endcap_key][chID] = {}
                for module in range(1,5):
                    output_dict[mv][endcap_key][chID][module] = {}
                    for VFAT in range(12):
                        output_dict[mv][endcap_key][chID][module][VFAT]={"num":0,"den":0}
    return output_dict
...
def passCut(PropHitonEta,etaPID,prop_hit_index,maxPropR_Err=0.7,maxPropPhi_Err=0.001,fiducialCutR=0.5,fiducialCutPhi=0.002,minPt=0.,maxChi2=9999999,minME1Hit=0,minME2Hit=0,minME3Hit=0,minME4Hit=0):
    passedCut = True
    station,region,chamber,layer,eta_partition = convert_etaPID(etaPID)
    if station == 1:
        etaP_bound = boundariesGE11
    if station == 2:
        etaP_bound = boundariesGE21
    ...
    #PhiMin = etaP_bound[etaPID][1]
    #PhiMax = etaP_bound[etaPID][0]
    ...
    #rMax = etaP_bound[etaPID][2]
    #rMin = etaP_bound[etaPID][3]
...
def generate2DEfficiencyPlotbyVFAT(EfficiencyDictVFAT,efficiency_target):
    ...
    if efficiency_target in ["ML1","ML2","PL1","PL2"]:
        ...
        #for chamber_number in range(16,17):
        for chamber_number in [16, 18]:
            chamberSize = "Short" if chamber_number%2 == 1 else "Long"
            
            region = 1 if endcapTag[0]=="P" else -1
            chamber_ID = getChamberName(region,chamber_number,endcapTag[-1],station=2) 
...
def generateEfficiencyPlot2DGE21(sourceDict,input_region=1,input_layer=1,debug=False):
    ## Transforming in list
    reg_tag_string = "All" if isinstance(input_region, list) else "P" if input_region == 1 else "M"
    lay_tag_string = "" if isinstance(input_layer, list) else "L1" if input_layer == 1 else "L2"
    input_region = [input_region] if not isinstance(input_region, list) else input_region
    ...
    title = "GE21"+reg_tag_string+lay_tag_string
    phi_nbins,phi_min,phi_max = 36,0.5,36.5
    etaP_nbins, etaP_min,etaP_max = 17, -0.5, 16.5    
    ...
    etaWise_eff_dict = {}
    for i in range(1,5):
        moduleWise_eff_dict[i] = {'num':0,'den':0}
    for j in range(1,17):
        etaWise_eff_dict[j] = {'num':0,'den':0}
    ...
    for etaPartitionID,value in sourceDict.items():
        ...
        try:
            ...
            N+=1
        mod = etaP2Module(eta)
        moduleWise_eff_dict[mod]['num'] += etaPartitionRecHits
        moduleWise_eff_dict[mod]['den'] += etaPartitionPropHits

        etaWise_eff_dict[eta]['num'] += etaPartitionRecHits
        etaWise_eff_dict[eta]['den'] += etaPartitionPropHits
    ...
    #print("module-wise: ",moduleWise_eff_dict)
    #print("eta-wise: ",etaWise_eff_dict)

    return EfficiencyTH2D,NumTH2D,DenTH2D,Summary


def generateEfficiencyPlot2DGE21_module(sourceDict,input_region=1,input_layer=1,debug=False):
    ## Transforming in list
    reg_tag_string = "All" if isinstance(input_region, list) else "P" if input_region == 1 else "M"
    lay_tag_string = "" if isinstance(input_layer, list) else "L1" if input_layer == 1 else "L2"
    input_region = [input_region] if not isinstance(input_region, list) else input_region
    ...
    title = "GE21"+reg_tag_string+lay_tag_string
    phi_nbins,phi_min,phi_max = 36,0.5,36.5
    mod_nbins, mod_min,mod_max = 5, -0.5, 4.5    

    EfficiencyTH2D = ROOT.TH2F(title+"_ChambersEfficiency_module", title+"_ChambersEfficiency_module", phi_nbins,phi_min,phi_max,mod_nbins,mod_min,mod_max)
    NumTH2D = ROOT.TH2F(title+"_ChambersNum_module", title+"_ChambersNum_module", phi_nbins,phi_min,phi_max,mod_nbins,mod_min,mod_max)
    DenTH2D = ROOT.TH2F(title+"_ChambersDen_module", title+"_ChambersDen_module", phi_nbins,phi_min,phi_max,mod_nbins,mod_min,mod_max)
    ...
    EfficiencyTH2D = NumTH2D.Clone()
    EfficiencyTH2D.SetTitle(title+"_ChambersEfficiency_module")
    EfficiencyTH2D.SetName(title+"_ChambersEfficiency_module")
    EfficiencyTH2D.Divide(DenTH2D)
    for x in range(1,phi_nbins+1):
        for y in range(1,mod_nbins+1):
            EfficiencyTH2D.SetBinContent(x,y,  round(EfficiencyTH2D.GetBinContent(x,y),2) )
    ...
    EfficiencyTH2D.GetXaxis().SetTitle("Chamber number")
    EfficiencyTH2D.GetXaxis().SetTitleSize(0.05)
    EfficiencyTH2D.GetYaxis().SetTitle("GEM Module")
    EfficiencyTH2D.GetYaxis().SetTitleSize(0.05)
    ...
    NumTH2D.GetXaxis().SetTitle("Chamber number")
    NumTH2D.GetXaxis().SetTitleSize(0.05)
    NumTH2D.GetYaxis().SetTitle("GEM Module")
    NumTH2D.GetYaxis().SetTitleSize(0.05)
    ...
    DenTH2D.GetXaxis().SetTitle("Chamber number")
    DenTH2D.GetXaxis().SetTitleSize(0.05)
    DenTH2D.GetYaxis().SetTitle("GEM Module")
    DenTH2D.GetYaxis().SetTitleSize(0.05)
...
def printSummary(sourceDict,matching_variables,ResidualCutOff,matching_variable_units,debug=False):
    print("sourceDict[]: ",set(list(sourceDict['glb_rdphi'].keys())))
    for matching_variable in matching_variables:
        ...
        for key in sourceDict[matching_variable].keys():
            station,region,chamber,layer,eta = convert_etaPID(key)
            num=0
            den=0
            for etap in range(1,17):
                if etap != eta: continue
                num = sum([sourceDict[matching_variable][key][k]['num'] for k in range(0,11) if etap == eta])
                den = sum([sourceDict[matching_variable][key][k]['den'] for k in range(0,11) if etap == eta])
            try:
                print("Efficiency GE21 ETA"+str(eta)+"  ==> ", num , "/",den, "\t = ", round(float(num)/float(den),3))
            except:
        ...
        try:
            print("Efficiency GE21 ",module,  "   ==> ", num , "/",den, "\t = ", round(float(num)/float(den),3))
        except:
...
if __name__ == '__main__':
    ...
    for k in boundariesGE21:
        print(convert_etaPID(k))
    pass

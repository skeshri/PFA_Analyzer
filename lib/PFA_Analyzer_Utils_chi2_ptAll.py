    ...
    for mv in matching_variables:
        ...
        for (re,la) in [(1,1),(1,2),(-1,1),(-1,2)]:
            ...
            for chamber in range(1,37):
                ...
                for eta in list(range(1,9))+["All"]:
                    titleTH1 = chID+"_eta"+str(eta)+"_"
                    output_dict[mv][endcap_key][chID][eta]={"Residual":ROOT.TH1F(titleTH1+"Residual",titleTH1+"Residual",nbins,-4,4)}
...
#def passCut(PropHitonEta,etaPID,prop_hit_index,maxPropR_Err=0.7,maxPropPhi_Err=0.001,fiducialCutR=0.5,fiducialCutPhi=0.002,minPt=0.,maxChi2=9999999,minME1Hit=0,minME2Hit=0,minME3Hit=0,minME4Hit=0,ID=2):
    ...
    #A = polar2cartesian(rMin + fiducialCutR ,PhiMin + fiducialCutPhi)
    #B = polar2cartesian(rMax - fiducialCutR ,PhiMin + fiducialCutPhi)
    #C = polar2cartesian(rMax - fiducialCutR ,PhiMax - fiducialCutPhi)
    #D = polar2cartesian(rMin + fiducialCutR ,PhiMax - fiducialCutPhi)
    #P = (PropHitx,PropHity)
    
    #res = lambda A, B: (A[0]-B[0],A[1]-B[1])

    A = polar2cartesian(rMin  ,PhiMin )
    B = polar2cartesian(rMax  ,PhiMin )
    C = polar2cartesian(rMax  ,PhiMax )
    D = polar2cartesian(rMin  ,PhiMax )
    P = (PropHitx,PropHity)
    #A1 = res(A,(5,-5))
    #B1 = res(B,(5,5))
    #C1 = res(C,(-5,5))
    ...
    #if not is_point_in_trapezoid(A, B, C, D, P):
    #    passedCut = -3

    #if (PropHitx > (x1+1) or PropHitx > (x2+1)) and (PropHitx < (x3-1) or PropHitx < (x4-1)):
    #    passedCut = -3
    #if (PropHity > (y1+1) or PropHity > (y4+1)) and (PropHity < (y3-1) or PropHity < (y2-1)):
    #    passedCut = -3
